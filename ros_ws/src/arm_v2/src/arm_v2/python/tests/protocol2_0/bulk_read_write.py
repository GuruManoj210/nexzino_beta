...
while 1:
    ...
    dxl_addparam_result = groupBulkWrite.addParam(
        DXL2_ID, ADDR_GOAL_POSITION, LEN_GOAL_POSITION, param_goal_position
    )
    if dxl_addparam_result != True:
    ...
    while 1:
        ...
        dxl2_present_position = groupBulkRead.getData(
            DXL2_ID, ADDR_PRESENT_POSITION, LEN_PRESENT_POSITION
        )

        # Get LED value
        #  dxl2_led_value_read = groupBulkRead.getData(DXL2_ID,  ADDR_PRESENT_POSITION, LEN_PRESENT_POSITION)

        print(
            "[ID:%03d] Present Position1 : %d \t [ID:%03d] Present Position %d"
